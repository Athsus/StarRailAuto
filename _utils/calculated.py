...
from .config import read_json_file, CONFIG_FILE_NAME, get_file
from .log import log
...
class Calculated:
    ...
    def __init__(self, title="崩坏：星穹铁道", det_model_name="ch_PP-OCRv3_det", rec_model_name="densenet_lite_114-fc",
                 number=False, start=True):
        """
        参数: 
    ...
    def take_screenshot(self, points=(0, 0, 0, 0)):
        ...
        log.debug(f"rect: {left}, {top}, {right}, {bottom}")
    ...
    def move(self, com=["w", "a", "s", "d", "f"], time1=1, map_name=""):
        ...
        if self.data.get("sprint", False) and (
                self.compare_lists(result, [130, 160, 180]) or self.compare_lists([200, 200, 200], result)):
            time.sleep(0.05)
            log.info("疾跑")
    ...
    def read_img(self, path, prefix='./documents/pictures/'):
        """
        说明：
            读取图像
        参数：
